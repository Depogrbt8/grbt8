...
export default function HeroSection({ activeService = 'flight', onServiceChange }: HeroSectionProps) {
  ...
  const getIconClasses = (service: ServiceType) => {
    ...
    return `rounded-full w-20 h-20 flex items-center justify-center mb-2 border-2 transition-all duration-200 hover:scale-[1.02] cursor-pointer pointer-events-auto bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] ${
      isActive ? 'border-white' : 'border-white/80'
    }`;
  };

  const getIconColor = () => 'text-green-600';
  ...
  return (
    <div className="bg-green-600 text-center text-white pb-5 sm:pb-32 pt-[2rem] sm:pt-8 relative z-10 rounded-b-[16px] sm:rounded-b-[32px]">
      ...
      {/* Yeşil bant: yuvarlak ikonlar ve etiketler için arka plan */}
      <div className="absolute left-0 right-0 bottom-0 translate-y-[28%] sm:translate-y-[48%] h-28 sm:h-36 bg-green-600 z-[11] rounded-t-2xl rounded-b-[16px] sm:rounded-b-[32px]" aria-hidden />
