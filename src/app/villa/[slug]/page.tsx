  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        description,
        type: 'website',
        url: `https://gurbetbiz.app/villa/${params.slug}`,
      },
      alternates: {
        canonical: `/villa/${params.slug}`,
      },
    };
  } catch (error) {
...
export default async function VillaPage({ params }: { params: { slug: string } }) {
  try {
    ...
    const breadcrumbItems = [
      ...
      { name: 'Villa Kiralama', url: 'https://gurbetbiz.app' },
      { name: keyword.keyword, url: `https://gurbetbiz.app/villa/${params.slug}` }
    ];
    ...
    return (
      <>
        ...
        />
        <LandingPageClient keyword={keyword.keyword} slug={params.slug} type="villa" />
      </>
    );
  } catch (error) {
    console.error('VillaPage error:', error);
    notFound();
  }
}
